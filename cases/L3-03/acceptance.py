...
CASE_ID = "L3-03"
CASE_VERSION = "1.0.0"
ALLOWED_CHANGES = {"aggregate.py"}
MAX_CHANGED_FILES = 1
CHECK_NAMES = ("hygiene.testsUnchanged", "hygiene.scopeRespected", "hygiene.changeBudget", "functional.visibleTests", "functional.nightTotals", "boundary.midnightNotDoubleCounted", "regression.eveningTransactionCounted", "boundary.nextDayExcluded")
HIDDEN_SCRIPT = """import json, sys
from pathlib import Path
sys.path.insert(0, sys.argv[1])
from aggregate import nightly_totals
transactions = json.loads(Path(sys.argv[1], "data", "transactions.json").read_text(encoding="utf-8"))
first = nightly_totals(transactions, "2026-02-10")
second = nightly_totals(transactions, "2026-02-11")
third = nightly_totals(transactions, "2026-02-12")
single = nightly_totals([{"account": "a", "amount": 5, "booked_at": "2026-02-11 00:00:00"}], "2026-02-10")
checks = {
    "functional.nightTotals": first == {"a": 7} and second == {"b": 11} and third == {"c": 7},
    "boundary.midnightNotDoubleCounted": single == {},
    "regression.eveningTransactionCounted": nightly_totals([{"account": "z", "amount": 2, "booked_at": "2026-02-10 23:59:59"}], "2026-02-10") == {"z": 2},
    "boundary.nextDayExcluded": nightly_totals([{"account": "z", "amount": 2, "booked_at": "2026-02-11 00:00:01"}], "2026-02-10") == {},
}
