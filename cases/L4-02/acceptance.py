...
CASE_ID = "L4-02"
CASE_VERSION = "1.0.0"
ALLOWED_CHANGES = {"events.py", "dispatcher.py", "projection.py", "pipeline.py"}
MAX_CHANGED_FILES = 4
CHECK_NAMES = ("hygiene.testsUnchanged", "hygiene.scopeRespected", "hygiene.changeBudget", "functional.visibleTests", "functional.eventTypeDeclared", "functional.dispatcherKnowsType", "functional.projectionIncludesType", "regression.existingTypesProjected", "boundary.unknownTypeRejected")
HIDDEN_SCRIPT = """import json, sys
sys.path.insert(0, sys.argv[1])
from dispatcher import registered_types
from events import EventType
from pipeline import run
handled, lines = run([{"type": "JOB_RETRYING", "id": "j-1"}, {"type": "JOB_COMPLETED", "id": "j-2"}])
rejected = False
try:
    run([{"type": "UNKNOWN"}])
except ValueError:
    rejected = True
checks = {
    "functional.eventTypeDeclared": EventType.JOB_RETRYING.value == "JOB_RETRYING",
    "functional.dispatcherKnowsType": EventType.JOB_RETRYING in registered_types(),
    "functional.projectionIncludesType": lines == [json.dumps({"type": "JOB_RETRYING", "id": "j-1"}, sort_keys=True), json.dumps({"type": "JOB_COMPLETED", "id": "j-2"}, sort_keys=True)],
    "regression.existingTypesProjected": handled == ["handled JOB_RETRYING", "handled JOB_COMPLETED"] and run([{"type": "JOB_SUBMITTED"}])[1] == [json.dumps({"type": "JOB_SUBMITTED"}, sort_keys=True)],
    "boundary.unknownTypeRejected": rejected,
}
