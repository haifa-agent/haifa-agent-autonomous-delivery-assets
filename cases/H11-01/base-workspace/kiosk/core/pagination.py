...
from kiosk.core.ordering import DEFAULT_ORDER

SEPARATOR = "|"


def _cursor_value(item: Item) -> str:
    """The value a cursor stores.

    Comparing two of these values must give the same answer as the listing order: the next page
    is everything that sorts after the cursor.
    """
...
def encode_cursor(item: Item, order: str = DEFAULT_ORDER) -> str:
    ...
    raw = SEPARATOR.join((order, _cursor_value(item), item.sku))
...
def _start_of_next_page(items: list[Item], value: str, sku: str) -> int:
    """Return the index of the first item that sorts after the cursor position."""
    for index, item in enumerate(items):
        if (_cursor_value(item), item.sku) > (value, sku):
            return index
    return len(items)
...
def page(
    ...
    if cursor is not None:
        ...
        start = _start_of_next_page(items, value, sku)
