...
CASE_ID = "L3-04"
CASE_VERSION = "1.0.0"
ALLOWED_CHANGES = {"importer.py"}
MAX_CHANGED_FILES = 1
CHECK_NAMES = ("hygiene.testsUnchanged", "hygiene.scopeRespected", "hygiene.changeBudget", "functional.visibleTests", "functional.largeImportCompletes", "functional.largeImportIsFast", "regression.sampleOrderPreserved", "boundary.duplicateInsideInput")
HIDDEN_SCRIPT = """import json, sys, time
from pathlib import Path
...
sample = json.loads(Path(sys.argv[1], "data", "rows.json").read_text(encoding="utf-8"))
sample_imported, sample_skipped = import_rows(sample, [{"id": "e-1", "name": "old"}])
checks = {
    "functional.largeImportCompletes": len(imported) == 8000 and skipped == [],
    "functional.largeImportIsFast": elapsed < 1.0,
    "regression.sampleOrderPreserved": [row["id"] for row in sample_imported] == ["n-1", "n-2"] and sample_skipped == ["e-1"],
    "boundary.duplicateInsideInput": import_rows([{"id": "d", "name": "1"}, {"id": "d", "name": "2"}], [])[1] == ["d"],
}
