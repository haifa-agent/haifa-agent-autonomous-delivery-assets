#!/usr/bin/env python3
"""Black-box acceptance for ladder case L1-05; prints one JSON line on stdout."""
...
CASE_ID = "L1-05"
CASE_VERSION = "1.0.0"
ALLOWED_CHANGES = {"search.py"}
CHECK_NAMES = (
    "hygiene.testsUnchanged",
    "hygiene.scopeRespected",
    "functional.visibleTests",
    "functional.asciiCaseInsensitive",
    "boundary.queryCaseFold",
    "boundary.noMatchAndEmptyQuery",
    "functional.orderPreserved",
)
HIDDEN_SCRIPT = """
import json, sys
sys.path.insert(0, sys.argv[1])
from search import matches
records = ["Mixed RECORD", "plain", "mixed-2"]
checks = {
    "functional.asciiCaseInsensitive": matches("mixed", records) == ["Mixed RECORD", "mixed-2"],
    "boundary.queryCaseFold": matches("PLAIN", records) == ["plain"] and matches("pLaIn", records) == ["plain"],
    "boundary.noMatchAndEmptyQuery": matches("absent", records) == [] and matches("", records) == [],
    "functional.orderPreserved": matches("mixed", records) == ["Mixed RECORD", "mixed-2"],
}
