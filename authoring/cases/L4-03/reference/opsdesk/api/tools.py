...
def describe_tools() -> list[dict[str, object]]:
    ...
    for definition in TOOLS.values():
        ...
        for parameter in definition.parameters:
            described_parameter: dict[str, object] = {"type": parameter.type, "description": parameter.description}
            for key in ("default", "minimum", "maximum"):
                if getattr(parameter, key) is not None:
                    described_parameter[key] = getattr(parameter, key)
            properties[parameter.name] = described_parameter
